...
public class PanneauTicTacToe extends JPanel implements Observateur {
    ...
    public PanneauTicTacToe(EtatPartieTicTacToe partie) {
        ...
        ecouteurTicTacToe = new EcouteurTicTacToe(null, null);
    ...
    public void seMettreAJour(Observable observable) {
        if (observable instanceof EtatPartieTicTacToe) {
            EtatPartieTicTacToe etat = (EtatPartieTicTacToe) observable;
            char[][] plateau = etat.getEtatPlateau();
            for (int i = 0; i < boutons.length; i++) {
                for (int j = 0; j < boutons[i].length; j++) {
                    char symbole = plateau[i][j];
                    if (symbole == '.') {
                        boutons[i][j].setIcon(null);
                    } else {
                        boutons[i][j].setIcon(ServiceImages.getIconePourSymbole(symbole));
                    }
                }
            }
        }
